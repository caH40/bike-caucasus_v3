...
const Pagination = ({
  ...
  const getClick = (item: '<<' | '>>' | number): void => {
    ...
    if (typeof item === 'number') {
      setPage(item - 1); // Переход на выбранную страницу.
      return;
    }
    setPage(0); // Переход на первую страницу.
  };
      ...
      <ul className={cx('list')}>
        ...
        {pages.length > 7 ? (
          ...
          pages.map((pageCurrent) => (
            <li
              className={cx('item', { active: page + 1 === pageCurrent })}
