...
v3.74.19 +title, description для страниц distances, distance;

-после удаления результата из протокола или при запуске обновления протокола не обновляется
отображаемая таблица протокола (места не изменяются), приходится делать обновление страницы в
браузере!;  
-необходимо к сущности Race привязывать \_id дистанции, для последующего использования его в
результате;  
-сделать проверку на дубликат названия дистанции, преобразовывать все в нижний регистр и
сравнивать с тем что сохранено;  
-подсказу об максимально возрасте 120 лет в категориях;  
